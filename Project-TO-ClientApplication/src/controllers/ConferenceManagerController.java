...
import application.enums.UsersRole;
...
public class ConferenceManagerController implements ControllerFunctionality {
	...
	private void addUserLabelsWithRoles(ObservableList<Label> ol, ArrayList<User> group, String role) {
		...
		for (User u : group) {
			...
			label = new Label(FeedPanelController.addNLsIfTooLong(title, 35));
	...
	@FXML public void initialize() {
		selectedConferenceId = TabsPanelController.getSelectedConferenceID();
		selectedConference = FeedPanelController.getConferenceByID(selectedConferenceId);
		...
		if (role != null && usersIds.size() > 0) {
			if (role == UsersRole.ORGANIZER || willThereBeAnyOrganizerLeft()) {
				...
				if (eventName.equals("setRoleSucceeded") || eventName.equals("expellSucceeded")) {
					...
					MainController.makeRequest(RequestType.UPDATE_CONFERENCE_FEED);
			...
			@Override public void run() {
				...
				try {
					selectedConference.getOrganizers().stream()
							.filter(o -> o.getId() == MainController.currentUser.getId()).
